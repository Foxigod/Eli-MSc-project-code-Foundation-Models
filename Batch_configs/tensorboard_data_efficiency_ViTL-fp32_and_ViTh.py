...
log_file = Path("./logs_batched/data_efficiency_ViTL-fp32_10seeds.log")
log_file2 = Path("./logs_batched/data_efficiency_ViTh_10seeds.log")
LIGHTNING_LOGS = Path("<./logs_batched/lightning_logs/>")
PDFILE = log_file.with_suffix(".pdf")
PDFILE = log_file.parent / "data_efficiency_ViTL-fp32_and_ViTh_10seeds.pdf"
...
with PdfPages(PDFILE) as pdf:
    for name, name_dict in names.items():
        ...
        for i, train_frac in enumerate(tfevents.keys()):
            ...
            plot_with_errors_to_axis(ax, iterations, iqmean_scalar_value, confidence_interval_scalar_value, colour=colours[i+1], label=f"ViT-L {train_frac}", white_region_below=True)
            tfevent_list = tfevents2[train_frac]
            iterations, iqmean_scalar_value, confidence_interval_scalar_value = extract_scalar_iqm_conf(tfevent_list, name=name, confidence_level=0.8, bootstrap_method="BCa")
            plot_with_errors_to_axis(ax, iterations, iqmean_scalar_value, confidence_interval_scalar_value, colour=colours[i+1], label=f"ViT-h {train_frac}", linestyle="--", white_region_below=True)
        ax.legend()
