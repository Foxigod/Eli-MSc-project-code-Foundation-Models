...
train_fractions=("0.5" "0.25" "0.125" "0.0625")
...
log_file="${log_dir}/data_efficiency_crops_updated_BaseRec.log"
...
for train_frac in "${train_fractions[@]}"; do
    for seed in "${seeds[@]}"; do
        ...
        output=$(sbatch --job-name="trfrac_${train_frac}__seed_${seed}_crops_updated_BaseRec" ./submit_ViT-B_sen4map-crops-updated_bs80_crop15_patch3_timepatch3_Recreate_baseline.sh \
          ...
          # --data.init_args.val_hdf5_keys_path <../TerraTorch_additions/>/key_files/Crop-classification/crop_val_keys_updated.pkl \
        jobid=${output//[^0-9]/}
        jobids+=("${jobid}")
